...
![](files/公文流转选择节点时添加办理时限日期选择-公文耗时开发%201.zip)

#### 案例

[复写table组件](./案例/复写table组件.md)
### 如何知道是什么组件
...
1. 需要对查询页面的 Table 组件进行参数获取（使用组件参数复写），获取到表格数据
2. 使用 `ModeList.getCheckedID()` 接口获取到全部的勾选id
3. 在全局添加一个记录勾选数据的字段
4. 当每次勾选时，就会进入到组件参数复写，就可以获取到表格的数据，然后根据勾选的数据id，取到当前页勾选的数据，添加到全局的记录勾选数据字段中
5. 可以在其它地方获取 `window.selectedData` 拿到勾选的数据
...
﻿[使用Chii工具调试手机端页面](./使用Chii工具调试手机端页面.md)

### 使用eruda调试手机端页面

[使用eruda调试手机端页面](./使用eruda调试手机端页面.md)﻿
## 第三方集成
...
1. 看浏览器控制台有没有抛出错误，看这个错误是哪个 ecode 引起的，取消发布出现错误的 ecode 应用，再看看能不能正常执行
2. 可能是系统的原因导致 ecode 不生效，尝试修改 register.js 文件，然后取消发布应用，再重新发布

## 案例

### 建模

﻿[ecode-重写建模查询菜单点击事件以及右键菜单点击事件](./案例/ecode-重写建模查询菜单点击事件以及右键菜单点击事件.md)

﻿[前端-案例-改变查询页面查询条件日期的值，并进行查询](./案例/前端-案例-改变查询页面查询条件日期的值，并进行查询.md)

### 流程

﻿[ecode添加流程顶部按钮](./案例/ecode添加流程顶部按钮.md)

[前端-案例-流程一键流转，点击按钮流转到指定节点](./案例/前端-案例-流程一键流转，点击按钮流转到指定节点.md)

﻿[新建流程页面添加页签（复写组件参数）](./案例/新建流程页面添加页签（复写组件参数）.md)
