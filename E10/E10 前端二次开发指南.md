...
解决方法是在按钮点击时通过 js 在浏览器 `会话存储` 中存入标识，然后调用流程保存，当流程保存刷新后，再用 js 判断是否有标识，有则执行触发动作，比如打开弹窗。  

	⚠️ 但这并不是一个很好的方法，因为在标识还没过期的时间内，如果有别的流程打开了流程页面，则会打开保存后的触发动作。可以改为向浏览器链接添加标识，新建保存刷新之后判断是否有这个标识再执行保存后动作。

示例代码：
按钮点击动作 js
```javascript
const SAVE_FLAG = 'ecode_after_new_flow_save';

// 会话中存入标识和时间
sessionStorage.setItem(SAVE_FLAG, String(Date.now()));
// 触发流程保存动作
window.weappWorkflow.getFlowPageSDK().doSave();
```
...
// 获取会话存储中的标识
const savedAt = Number(sessionStorage.getItem(SAVE_FLAG));
// 判断是否有标识，并获取标识中的时间，该标识只在 10 内生效，判断当前时间是否超过标识时间 10 秒
if (savedAt && Date.now() - savedAt < 10 * 1000) {
...
**如何实现**  
需要在 eb 代码块中做一个保存后的钩子动作，保存后触发流程表单刷新。由于 eb 表是在弹窗的 iframe 打开，所以需要获取上层的窗口才能获取到流程表单的 sdk，来执行流程刷新。
 ...
 * 查找流程父页面表单 SDK
 */
function findParentFormSdk() {
    let targetWindow = window.parent;

    while (targetWindow && targetWindow !== window) {
        try {
            const formSdk = targetWindow.WeFormSDK?.getWeFormInstance();

            if (formSdk?.reloadPage) {
                return formSdk;
            }
            ...
            console.error('二开：查找流程父页面 WeFormSDK 出错', e);
            break;
        }
    }
...
ebFormSdk.registerAction(window.WeFormSDK.ACTION_FORM_SAVE, () => {
    const flowFormSdk = findParentFormSdk();

    if (!flowFormSdk) {
        return;
    }

	// 需要加上延迟，因为 eb 保存后会触发更新动作流，动作流执行需要时间，所以需要延迟
	// 后触发流程表单刷新
    window.setTimeout(() => {
	    // 刷新流程表单
        flowFormSdk.reloadPage();
        console.log('二开：EB 表保存完成，延迟刷新流程表单');
    }, 1000);
});
```
