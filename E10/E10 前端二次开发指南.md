...
**应用ID**：
右键点击 E-Code 应用，选择 `属性` 可查看应用信息，其中应用ID分为 `appId` 和 `appUUID` , 需要注意 `appId` 是可变的，导入到其它 E10 系统后 ID 可能会变，而 `appUUID` 是不变的，所以在代码中需要使用应用 ID 时，尽量使用 `appUUID`。
...
#### 新建流程页面通过自定义按钮触发保存，保存后触发动作

**需求描述**  
在流程表单上创建一个自定义按钮，在新建流程页面点击按钮后保存流程，然后触发弹窗，弹窗显示 eb 表单新建页面

**如何实现**  
需要做新建流程保存后触发动作，与编辑保存不同，新建保存会刷新页面，也就不能通过 js 的钩子动作来实现保存后触发。
解决方法是使用逻辑流进行保存，保存之后打开 eb 窗口。

流程表单创建按钮：
在流程表单内放入自定义按钮，单击事件选择逻辑流

![](files/Pasted%20image%2020260901090843.png)

逻辑流设计：
在逻辑流中保存流程表单，可以在保存后面设计动作，让它保存后能执行这些动作，比如打开 eb 窗口

![](files/Pasted%20image%2020260901091106.png)

![](files/Pasted%20image%2020260901091117.png)

#### 流程内弹窗中的 eb 表保存后刷新流程

**需求**
在流程内通过弹窗打开 eb 表单，eb 表单保存后会执行动作流更新流程数据，需要 eb 表保存后在流程中刷新，让流程中能看到更新的数据。

**如何实现**  
在流程中打开的 eb 表单的弹窗，需要使用标准功能打开，比如上面使用逻辑流打开 eb 弹窗。  
在流程里打开 eb 弹窗时，需要设置一个标记，用于在 eb 表单判断是否刷新流程，例如：`window.workflowRefreshFlag = true;`

使用上面的打开 eb 弹窗示例，打开前执行了代码，设置了一个标记。

![](files/Pasted%20image%2020260901091616.png)

需要在 eb 代码块中做一个保存后的钩子动作，保存后触发流程表单刷新。
eb 表代码块，示例代码：
```javascript
const ebFormSdk = window.WeFormSDK.getWeFormInstance();
// 表单保存后刷新当前流程表单，只有在流程中打开当前 eb 表单时才会生效
ebFormSdk.registerAction(window.WeFormSDK.ACTION_FORM_SAVE, () => {
    // 获取在流程中打开 eb 表单时设置的标识，判断是否为在流程中打开
    if (window.workflowRefreshFlag) {
        window.workflowRefreshFlag = false;
        // 需要延迟，因为保存后需要等动作流执行完成
        window.setTimeout(()=>{
            const flowFormSdk = window.weappWorkflow.getFlowPageSDK();
            // 刷新流程表单
            flowFormSdk.reloadPage();
            console.log('二开：EB 表保存完成，已刷新流程表单');
        },1000)
...
在后台流程设置中，先按 f12 打开开发者工具并切换到网络标签页，然后在流程设置中切换到流转设置标签页，在开发者工具的网络标签中查看 `getNodeList` 接口的返回数据，找到 `data` 数据中的对应节点的id。

![](files/Pasted%20image%2020260818102120.png)

#### 方法三：使用 formSDK 获取当前节点id

代码示例：

```javascript
const sdk = window.WeFormSDK.getWeFormInstance();
const formInfo = sdk.getBaseInfo();
const nodeId = formInfo.customParam.nodeId
```
...
包含表单全部字段数据和表单页面布局  
接口地址：`/api/workflow/core/form/core/getFormDataAndLayout?formId=1151612316977602563&dataId=1247741669520719874 
...
可以使用 `FormSwitch` 组件复制字段，比如复制整个明细，详细可以看 [FormSwitch - 表单组件分发](#FormSwitch%20-%20表单组件分发)

### ebuilderSDK.openPage() - eb 表单弹窗

使用场景：需要使用弹窗打开 eb 表单页面，比如在流程中弹窗打开 eb 表单页面

使用此接口可以使用标准的 eb 弹窗打开 eb 表单页面，此接口也可以打开任意的链接，不限定 eb 表单页面

示例：

```javascript
const ebSdk = window.ebuilderSDK;
const url = `/sp/ebdfpage/card/2/1307107355997863937/${id}`;
const options = {
    openType: 3,
    modalWidth: '80%',
    modalHeight: '80%'
};
ebSdk.openPage(url, {}, options, () => {
        console.log(`二开：关联 EB 表单已保存，glbdxx：${id}`);
    });
```

- options 参数可配置弹窗
	- `modalWidth` 和 `modalHeight` 可配置弹窗的宽高，可用像素或百分比
	- openType 可配置弹窗的打开方式，方式为：
		- 1  在当前页面打开，也就是在当前页面跳转到指定页面
		- 2  在浏览器的新页面打开
		- 3  使用弹窗打开
- openPage 的第 4 个参数（最后一个）为 eb 表单保存后的回调

效果：
![](files/Pasted%20image%2020260901165252.png)
...
此组件可用于在表单中复制已有的表单字段，比如复制出一个已有的明细
组件说明：[e-cology.com.cn/sp/form/components/FormSwitch?ticket=ST-7374-UZ1hsJZqHEs3adc7Vmz1-https://www.e-cology.com.cn](https://www.e-cology.com.cn/sp/form/components/FormSwitch)

案例：[Ecode-表单明细树组件](Ecode-表单明细树组件.md)
...
![](files/Pasted%20image%2020260713115708.png)

![](files/Pasted%20image%2020260713115733.png)

#### 使用方法
以下是如何复制一个表单字段的用法：
需要使用 `CorsComponent` 组件，在 `compName` 参数中指定 `FormSwitch` , 示例：
...
**config 参数** ：
`config` 参数是字段的属性，决定这个字段是什么样的，可以使用 `formSdk.getFieldInfo(fieldId)` 来获取字段属性，放到 `config` 参数中，其中里面的 `readOnly` 和 `disabled` 决定了这个字段的只读属性，如果字段只读需要将这两个属性赋值为 true
**请注意**：通过 `formSdk.getFieldInfo(fieldId)` 获取的 `readOnly` 并不准确 ,请使用表单 sdk 来获取字段编辑属性

**value 参数** ：
参数格式参见文档
该参数表示字段的值，通过表单 SDK 获取的字段值需要进行转换，才能放入 value 参数中，转换示例：
...
**onChange参数**：
当值改变时会调用 onChange(value) ，value 为变化后的值，与组件的 value 参数的值格式一样。
示例：
...
﻿[ecode-流程签字意见放到右边](案例/ecode-流程签字意见放到右边.md)

### 流程页面添加按钮

[案例-流程表单页面添加按钮](案例/案例-流程表单页面添加按钮.md)

### 明细表树组件
[Ecode-表单明细树组件](案例/Ecode-表单明细树组件.md)
