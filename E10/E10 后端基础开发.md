...
### Debug 调试
[E10二开服务windows开启debug调试端口方法，在编辑器进行debug](E10二开服务windows开启debug调试端口方法，在编辑器进行debug.md)
...
Action 具有输入和输出参数，对应 Action 后端中传入的参数和输出的参数。在输入参数中可通过动态赋值，来获取表单字段值或者流程系统参数。输出参数可在动作流后面进行获取，比如说输出参数返回的是成功还是失败，在动作流进行流转控制，或者是在动作流获取输出参数中的结果。
...
注意输出参数只能配置 Action 返回的 `WeaResult` 中的 `data` 对象的数据
### 创建 Action
...
Action 通过 `WeaResult.fail()` 返回的错误消息在动作流中是获取不到的，因为 Action 组件的输出参数只能获取 `WeaResult` 中的 data 数据，那怎么获取 Acton 的错误消息并在前端进行提示呢。

在后端中 Action 返回时需要在 `WeaResult` 的 data 中存入 `是否成功` 和 `返回消息` 这两个数据，这些数据用于存入业务执行是否成功的数据，然后在 Action 的输出参数中进行配置：

![](files/Pasted%20image%2020260814112525.png)

在 Action 组件需要进行异常处理，选择出错时跳过节点，这样它就不会报错时停留在当前组件，可以流转到后面的条件控制中

![](files/Pasted%20image%2020260814112557.png)

![](files/Pasted%20image%2020260814112811.png)

然后在 Action 组件后面做条件控制，当 Action 返回的参数中的 `是否成功` 为 false 时，则进入终止组件，需要在组件中配置提醒内容，提醒内容中添加 Action 输出参数中的 `返回消息`

![](files/Pasted%20image%2020260814112838.png)

错误提示效果：
![](files/Pasted%20image%2020260814113142.png)
