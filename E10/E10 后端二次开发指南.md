...
开启方法见：[E10二开服务windows开启debug调试端口方法，在编辑器进行debug](E10二开服务windows开启debug调试端口方法，在编辑器进行debug.md)

## 基础开发

包含接口开发、数据库操作、缓存、日志、action 开发等。  
[E10 后端基础开发](E10%20后端基础开发.md)
...
注意：Action 的 `Service` 注解的名称必需与类名一致，不然 jar 包上传校验不通过，会提示配置文件中找不到该名称的 Action
...
如果参数配置为文本类型和数组，则可以传入多个 List 类型的表单字段值（如人力资源、附件上传），参数的值会将表单字段值合并为数组，例如：`["1343363622","232567674545"]`

![](files/Pasted%20image%2020260807101713.png)
![](files/Pasted%20image%2020260807101756.png)
...
com.weaver.seconddev 包下面的日志级别都是info
...
`@ConfigurationProperties` 注解可以配置属性名前缀，例如下面的示例中读取的几个配置属性为：
- agency-draw.purchase-table.table-name
- agency-draw.purchase-table.project-name-field
- agency-draw.purchase-table.project-num-field

注意，使用该注解时属性名的命名规范必需使用**小写短横线**命名，不然会读取不到属性。  
读取到配置属性值后会赋值到对象属性中，可通过在类属性中赋值来定义默认值。
...
@Data  
@Configuration  
@ConfigurationProperties(prefix = "agency-draw.purchase-table")  
public static class PurchaseTable {  
    /**  
     * 表名  
     */  
    private String tableName = "uf_cgxqxmtz";  
    /**  
     * 项目名称字段名  
     */  
    private String projectNameField = "cgxmmc";  
    /**  
     * 项目编号字段名  
     */  
    private String projectNumField = "xmbh";  
}
```
...
文件上传、替换、下载、复制，文档：[技术文档-文件服务SDK能力](https://weapp.eteams.cn/build/techdoc/wdoc/index.html#/public/doc/cd16e1dc-75be-4848-88ae-092fe1d0f8ee)
...
获取文件信息以及附件操作功能，官方文档：[技术文档-文件RPC接口](https://weapp.eteams.cn/build/techdoc/wdoc/index.html#/public/doc/07d32f63-779d-4b3a-89b1-5c08ed3b0876)

##### 获取文件信息
获取文件信息返回的 `FileObj` 对象示例：

```json
{
  "ban": false,
  "createTime": 1785829818000,
  "deleteType": 0,
  "doc": true,
  "docId": 7518582981846566416,
  "docType": "office",
  "encryptId": "68C10EA97CDD7C934AF5BDF18DF0649FD048406DFD859BD3F4A6781F4D64FFA51",
  "extName": "docx",
  "fileid": 1299785171338174467,
  "hasRef": false,
  "id": 1299785171338174467,
  "image": false,
  "img": false,
  "module": "workflow",
  "name": "jointFile.docx",
  ...
  "signTip": "",
  "size": 17953,
  "tenantKey": "thhtqh9ryi",
  "type": "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
  "updateTime": 1785829819000,
  "uploadEmployeeId": 7515108099660413316,
  "uploadName": "姚礼林",
  "uploadTime": 1785829818000,
  "uploadUser": 7515108099660413316,
  "uploadUserName": "姚礼林",
  "url": "4f11475946fb432994b92a00ac1db543"
}
```
...
#### UserService - 获取用户信息
`com.weaver.workflow.common.cfg.org.service.UserService` 该类和 `HrmCommonEmployeeService` 类差不多

**注意：** 当方法要传入租户key时，不可传控或 null ,会导致查询不到数据，因为需要将租户 key 作为 sql 查询条件
...
明细表中的 `form_data_id` 字段可用于关联主表的id  
示例：
```sql
-- 主表和明细表通过 form_data_id 关联
SELECT 
    m.*,
    d.*
FROM 
    formtable_main_xxx m   -- 主表，xxx为实际表编号
    INNER JOIN formtable_main_xxx_dt1 d  -- 明细表，xxx为实际表编号，dt1为明细表序号
    ON m.id = d.form_data_id
WHERE 
    m.id = 123;  -- 替换为实际的主表数据ID
```
...
#### employee - 人员信息表
该表存放所有人员数据，注意该表在 `eteams` 数据库中，查询该表需要在此数据库中进行查询

### 表单
#### form - 表单信息

表单定义主表。可根据表单 ID 查询表单元数据，并通过 `id` 与 `form_table.form_id` 关联获取实际业务数据表名。

主要字段：

| 字段           | 说明                                        |
| ------------ | ----------------------------------------- |
| `id`         | 表单 ID，表单的主键；关联 `form_table.form_id`。      |
| `target_id`  | 表单目标 ID；`FormInfoServiceImpl` 查询表单时会返回该值。 |
| `tenant_key` | 租户标识。多租户场景查询时需作为条件传入。                     |
| `is_delete`  | 逻辑删除标识；查询有效表单时使用 `is_delete = 0`。         |
表单id 可从表单页面中获取

![](files/Pasted%20image%2020260710094741.png)
#### form_table - 表单物理表映射

保存表单与其业务数据表（主表或明细表）的映射关系。通过 `form.id = form_table.form_id` 可取得表单对应的物理表名。

主要字段：

| 字段 | 说明 |
| --- | --- |
| `form_id` | 表单 ID，关联 `form.id`。 |
| `table_name` | 表单对应的业务数据表名；可用该字段反查表单。 |

#### sub_form - 明细表单信息

保存明细表单定义。可通过明细业务表名（`data_key`）取得明细表单 ID，或取得其所属主表单 ID。
...
| `id` | 明细表单 ID。 |
| `form_id` | 所属主表单 ID。 |
| `data_key` | 明细表对应的业务数据表名。 |
| `tenant_key` | 租户标识。多租户场景查询时需作为条件传入。 |
| `is_delete` | 逻辑删除标识；查询有效明细表单时使用 `is_delete = 0`。 |

示例：根据表名查询表单或明细表单信息。

```sql
-- 查询主表单及其物理表名
SELECT f.id, f.target_id, ft.table_name
FROM form f
JOIN form_table ft ON f.id = ft.form_id
WHERE ft.table_name = '实际业务表名'
  AND f.is_delete = 0;

-- 查询明细表单及所属主表单
SELECT id, form_id
FROM sub_form
WHERE data_key = '实际明细业务表名'
  AND is_delete = 0;
```
...
该表可获取文件信息，示例：
```sql
SELECT name FROM fileobj WHERE id=?
```

#### file_storage_info - 文件存储信息

获取文件存储路径示例：
```sql
SELECT file_path FROM file_storage_info WHERE file_url = ?
```

file_url 可通过 `fileobj` 表的 `url` 字段获取
...
### 上传二开 jar 包提示 xml 配置文件中的 bean 不存在

确认是否使用了 @Service("name") 注解，并且注解中的值与 xml 配置文件中的名称一致，如果正确可能是检测的问题，先将 xml 配置文件中的 bean 注释，然后再上传 jar 包
...
### 运维手册
参考：[E10常见运维措施](https://www.e-cology.com.cn/sp/doc/docDetail/1108321353053364387)

### 服务器 IP 变更，修改 IP 地址

服务器 IP 变更后，会导致 E10 无法启动，需要变更 E10 的 IP 地址。

**操作方法**  
- 修改运维平台中的 IP：到运维平台中修改 IP，路径：日常维护 > 系统&配置 > 服务器IP变更 ,修改为新的 IP 地址，修改后运维平台内会根据新的 IP 检测服务状态，但这还没有修改 E10 内的 IP ，需要在 E10 中修改配置文件。  
- 修改 E10 配置文件：修改该路径 `weaver\E10\webapps\ROOT\WEB-INF\classes\weaver\config\config-center` 中配置文件中的 IP，只要配置文件中包含旧 IP 的都进行修改，可以使用 AI 进行修改，提示词（需先替换提示词中的 {} 的内容）：
```
修改 {E10部署路径}\weaver\E10\webapps\ROOT\WEB-INF\classes\weaver\config\config-center 中所有配置文件中的ip地址，将含有 {原IP地址} 的属性值中替换为 {旧IP地址}，注意只需要替换属性值中的ip，而不是把属性值修改为这个ip，如果属性值包含其它文本则需要保留，修改完后列出修改的配置文件名称以及原属性值，替换后的属性值。需要对涉及修改的配置文件进行备份。
```
- 修改二开服务中的配置文件：在 `weaver\weaver-secondev-service\webapps\ROOT\WEB-INF\classes\weaver\config\config-center` 路径中需要替换所有含有旧 IP 的配置文件中的属性值，替换为新 IP，可使用 AI 进行替换，提示词（需先修改提示词中 {} 的内容）：
```
在该路径下 {部署路径}\weaver\weaver-secondev-service\webapps\ROOT\WEB-INF\classes\weaver\config\config-center 将含有 {旧IP} ip的配置属性值替换为 {新IP}，仅替换该 ip，保留其它配置属性中的文本，替换前需要备份文件，全部替换后列出涉及修改的配置文件以及修改的属性值

```
- 修改 Nginx 旧 IP：修改 Nginx 配置文件中的 旧 IP，将 `weaver\nginx\conf.d\upstream.conf` 配置文件下的旧 IP 替换为新 IP
- 修改E10 和 二开服务的 java 启动参数中的 ip，这个是为了跨服务远程调用时可以正常调用，
	- linux 修改启动参数方法：找到 E10 的启动参数，在 E10 的部署路径中 `weaver\E10\bin\catalina.sh` ，在文件中找到 export JAVA_OPTS ，把里面的 旧 IP 替换为新 IP，修改二开服务的启动参数，找到 `weaver\weaver-secondev-service\bin\catalina.sh` 文件，把里面的 旧 IP 替换为新 IP
	- windows 修改方法：打开注册表，找到 `计算机\HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Apache Software Foundation\Procrun 2.0` , 修改 E10 和 二开服务中的 `Parameters` 中的 `java` 的 `Options` ，把里面的 旧 IP 替换为新 IP
	- 修改好启动参数就行了，参考文档： [E10 e-code 二开代码问题排查手册 - 飞书云文档](https://weaverecode.feishu.cn/docx/IwTQd3omCoBicmxFMcVcHSiHn5b)  文档中的 `5.instance does not exist`

以上完成之后重启 E10 以及所有周边服务