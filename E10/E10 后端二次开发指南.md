...
![](files/Pasted%20image%2020260616115358.png)

2.将开放平台提供的SDK jar包放到本地开发环境的依赖中，并放入二开服务的\webapps\ROOT\WEB-INF\lib 中

3.获取token
编写获取token的类，可将 token 存入缓存中
...
public class OpenApiTokenManger {
    private static final String STATE = "devTest";
    @Autowired
    private BaseCache baseCache;
    @Autowired
    ...
    public String fetchAccessToken() {
        if (baseCache.get("seconddev","openApiAccessToken") != null) {
            return (String) baseCache.get("seconddev","openApiAccessToken");
        }
        AccessTokenParam tokenParam = new AccessTokenParam(properties.getAppKey(), properties.getAppSecret(), "authorization_code", fetchCode());
        AccessToken token = AuthService.getAuthAccessToken(tokenParam, properties.getServerAddress()+"/papi/openapi", null);
        baseCache.set("seconddev","openApiAccessToken",token.getAccessToken(),token.getExpiresIn());
